...
function sort_cemetery_records_by_title( $query ) {
    // We only want to affect the main query on the front-end for the 'cemetery_record' archive.
    if ( ! is_admin() && $query->is_main_query() && is_post_type_archive( 'cemetery_record' ) ) {

        // Set the query to order posts by title.
        $query->set( 'orderby', 'title' );

        // Set the order to ascending (A-Z).
        $query->set( 'order', 'ASC' );
    }
}
